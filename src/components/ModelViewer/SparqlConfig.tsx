...
export function SparqlConfig({
	...
	const isSingleColumnLayout = useMediaQuery("(max-width: 99.99rem)");
	const hasExecutableQuery = queryText.trim().length > 0;
	...
	return (
		<div aria-label="SPARQL configuration" className="flex flex-1 p-4">
			<div className="flex w-full flex-col gap-4">
				<Accordion
					disableGutters
					expanded={isSingleColumnLayout ? isConfigExpanded : true}
									...
									<LabeledBorderBox label="named graph">
										...
										<ClearableInput
											...
											onChange={(event) => {
												dispatchModelState({
													payload: {
														sparql: { namedGraph: event.target.value },
													},
													type: "state/setSparqlConfig",
												});
											}}
														...
														dispatchModelState({
															payload: {
																sparql: {
																	orderBy: [
																		sparql.orderBy[0],
																		event.target.value as OrderByDirection,
																	],
																},
															},
															type: "state/setSparqlConfig",
									...
									<div className="relative min-h-[18rem] overflow-hidden rounded-panel border border-ui-border">
										<div
											...
											dangerouslySetInnerHTML={{
												__html: highlightedPydanticHtml,
											}}
										/>
										<textarea
